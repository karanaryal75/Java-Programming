...
public class MembershipManager implements Manager {
    ...
    public void addMember(AccountsManager accountsManager,
        ...
        if(person.getAge() >= 18){
            ...
            {
                Member newGoldMember = new GoldMember(person, getFreshMemberId(), account);
                members.add(newGoldMember);
            }
            else{
                Member newStandardMember = new StandardMember(person, getFreshMemberId(), account);
                members.add(newStandardMember);
            }
        }
        else{
    ...
    public Member lookupMember(Person person) throws MemberNotFound {
        for(Member containMember : members)
        {
            if (containMember.isPerson(person)){
    ...
    public void removeMember(AccountsManager accountsManager,
                             Person person) {
        try {
            Member hasMembership = lookupMember(person);
            accountsManager.removeAccount(hasMembership);
            members.remove(hasMembership);
