...
public class LoansManager implements Manager {
    ...
    public boolean processNewLoanRequest(Person person,
                                         ...
                                         Account account,
                                         double amount) {
        double x = (person.getDebt() + account.loanBalance());
        double y = (account.balance() + (person.getMonthlyIncome()*12));

        double debtCalc = x / y;
        if(person.getAge() >= 18){
            if (account.loanBalance() <= 10000){
                return true;
            }
            else if(debtCalc <= 1 && person.getCreditScore() >= 600 &&
                    (account.loanBalance()> 10000 && account.loanBalance() < 100000))
            {
                return true;

            }
            else if(debtCalc <= 0.5 && person.getCreditScore() >= 700 &&
                    (account.loanBalance()> 100000 && account.loanBalance() < 1000000))
            {
                return true;

            }
            else if (debtCalc == 0 && person.getCreditScore() >= 800
